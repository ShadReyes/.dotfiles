...
async function secureEnvFile(contents = "LINEAR_API_KEY=lin_api_dummy\n") {
  ...
  const envFile = path.join(directory, "fluid.env");
...
test("loadCredential rejects a symlinked credential directory", async () => {
  ...
  await assert.rejects(
    loadCredential(profile(path.join(linkedDirectory, "fluid.env"))),
    /symlink/i,
  );
});
...
test("executeVerifiedOperation runs only after preflight and never returns the credential", async () => {
  ...
  const fetchImpl = async (url, init) => {
    ...
    if (calls.length === 1) {
      return jsonResponse({
        data: {
          viewer: { id: "viewer-fluid", organization: EXPECTED_ORGANIZATION },
        },
      });
    }
...
test("locked CLI arguments reject profile overrides and ambient selection", () => {
  assert.throws(
    () => parseCliArgs(["--profile", "other.json", "query.graphql"], { lockedProfilePath: "fluid.json" }),
    /locked/i,
  );
  assert.deepEqual(
    parseCliArgs(["query.graphql", "--variables", '{"id":"DEV-1"}'], {
      lockedProfilePath: "fluid.json",
    }),
    {
      envelope: false,
      pretty: false,
      profilePath: "fluid.json",
...
test("locked CLI always emits verified workspace metadata", async () => {
  ...
  const responses = [
    { data: { viewer: { id: "viewer-fluid", organization: EXPECTED_ORGANIZATION } } },
    { data: { viewer: { id: "viewer-fluid" } } },
  ];
  let output = "";
  let errorOutput = "";
